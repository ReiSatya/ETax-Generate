...
import id.etax.service.PDFEngine.myCellAble;
		...
		for (String n : no) {
			...
			for (String nIn : noIn) {
				...
				if (!nIn.equals("kosong")) {
					...
					if (SQLData.checkSummary(tes1, period) == false) {
...
//						}
	...
	public String getData(String name1, String period) {
		...
		try {
			...
			String sourcePDF = Param.getPdfSource()+"summary.jasper";
			String pathReport = "";
//			if (name1.contains("/")) {
//				name1 = name1.replace("/", "//");
//			}
			String query = "SELECT DISTINCT \"tax_nama\",\"cif\" ,\"tax_npwp\", \"tax_alamat\", "
					+ "\"tax_jenispenghasilan\", \"tax_jumlahbunga\", \"tax_tarif\", \"tax_pphpotong\", \"tax_namabank\" "
...
//			pathReport = Param.getPdfPath() + period + "/"; //pake yg ini kalo di server linux
			...
			path = pathReport +"Summary_"+ name + ".pdf";
...
//				duplicate.get(name1)
				...
				path = pathReport + "Summary_" + name+" "+no+".pdf";
